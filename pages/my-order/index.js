...
export default function Home({ orders }) {
  console.log('orders list from ssr', orders);
      ...
      if (confirmDelete) {
        dispatch(deleteOrder(orderId))
          .then(() => {
            ...
            setOrdersList(prevOrders => prevOrders.filter(order => order._id !== orderId));
          })
          .catch(error => {
      ...
      <>
        ...
        <div className="mt-8 max-w-md mx-auto p-6 bg-white rounded-md shadow-md">
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div>
              {filteredOrders.map((order, index) => {
                const { _id, date, location, mealTypes, createdAt } = order;
                return (
                  <div key={index} className="mb-4 border p-4 rounded-md">
                    ...
                    <p className="text-gray-600">Ordered at {new Date(createdAt).toLocaleDateString('en-GB')}</p>
                    {mealTypes && (
                      <div className="flex">
          ...
          <div className="mb-4">
            ...
            <p className="text-lg font-semibold">Total Cost for All Orders: ₹{calculateTotalCost(ordersList)}</p>
          </div>
